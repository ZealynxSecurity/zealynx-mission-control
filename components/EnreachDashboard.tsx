...
export function EnreachDashboard({ className = '' }: EnreachDashboardProps) {
  ...
  const sorted = [...leads].sort(
    (a, b) => (URGENCY_ORDER[a.action_urgency] ?? 9) - (URGENCY_ORDER[b.action_urgency] ?? 9)
  );
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="card">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-sm font-medium text-[var(--color-text-muted)]">Total Leads</p>
              <p className="text-2xl font-bold text-[var(--color-text-primary)]">{total}</p>
            </div>
          </div>
          <p className="text-xs text-[var(--color-text-subtle)]">Enreach outbound pipeline</p>
        </div>
        ...
        <div className="card">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-sm font-medium text-[var(--color-text-muted)]">Follow Up Today</p>
              <p className="text-2xl font-bold text-[var(--color-text-primary)]">{todayCount}</p>
            </div>
          </div>
          <p className="text-xs text-[var(--color-text-subtle)]">Needs action now</p>
        </div>
        ...
        <div className="card">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-sm font-medium text-[var(--color-text-muted)]">This Week</p>
              <p className="text-2xl font-bold text-[var(--color-text-primary)]">{thisWeekCount}</p>
            </div>
          </div>
          <p className="text-xs text-[var(--color-text-subtle)]">Due within 7 days</p>
        </div>
        ...
        <div className="card">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-sm font-medium text-[var(--color-text-muted)]">Stalled</p>
              <p className="text-2xl font-bold text-[var(--color-text-primary)]">{stalledCount}</p>
            </div>
          </div>
          <p className="text-xs text-[var(--color-text-subtle)]">21+ days no contact</p>
        </div>
      </div>
      ...
      {todayLeads.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full border bg-[var(--color-warning)]/10 border-[var(--color-warning)]/30 text-[var(--color-warning)] font-semibold">
              {todayLeads.length}
            </span>
          </div>
          <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
            {todayLeads.map(lead => (
              <TodayCard key={lead.id} lead={lead} />
            ))}
          </div>
        </div>
      )}
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs px-2 py-0.5 rounded-full border border-[var(--color-border-primary)] text-[var(--color-text-subtle)]">
            {filtered.length}
          </span>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--color-border-primary)] bg-[var(--color-bg-tertiary)]">
                    <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-subtle)] uppercase tracking-wide">Company</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-subtle)] uppercase tracking-wide">Contact</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-subtle)] uppercase tracking-wide">Last Contact</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-subtle)] uppercase tracking-wide">Urgency</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-subtle)] uppercase tracking-wide">Status</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-subtle)] uppercase tracking-wide hidden lg:table-cell">Last Message</th>
                  </tr>
                </thead>
                  ...
                  {filtered.map(lead => (
                    <tr key={lead.id} className="hover:bg-[var(--color-bg-tertiary)]/50 transition-colors">
                      <td className="px-4 py-3">
                        <span className="font-medium text-[var(--color-text-primary)]">{lead.company}</span>
                      </td>
                      <td className="px-4 py-3 text-[var(--color-text-muted)]">
                        {lead.contact_username ?? lead.contact_name ?? '—'}
                      </td>
                      <td className="px-4 py-3 text-[var(--color-text-muted)]">
